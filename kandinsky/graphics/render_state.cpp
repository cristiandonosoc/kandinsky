...
void SetLights(RenderState* rs, Span<Light> lights) {
    for (Light& light : lights) {
        const Transform& transform = GetTransform(&light);
        switch (light.LightType) {
            ...
            case ELightType::Spotlight: {
                ...
                light.Spotlight.RS_InnerRadiusCos = Cos(ToRadians(light.Spotlight.InnerRadiusDeg));
                light.Spotlight.RS_OuterRadiusCos = Cos(ToRadians(light.Spotlight.OuterRadiusDeg));
                break;
            }
...
void SetUniforms(const RenderState& rs, const Shader& shader) {
    ...
    // clang-format off
	for (const Light& light : rs.Lights) {
...
void SetEntity(RenderState* rs, EntityID id) { rs->EntityID = id; }
